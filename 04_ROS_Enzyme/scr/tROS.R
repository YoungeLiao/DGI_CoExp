...
tROS <- data.frame(read_excel('./04_ROS_Enzyme/data/tROS.xlsx'))
head(tROS)
...
library(ggplot2)
library(ggpubr)
results <- compare_means(Rate~Light, data=tROS, group.by = 'Time',# ref.group = 'Dark', 
              method = 't.test')
...
p <- ggboxplot(tROS, x="Time", y="Rate", color = "Light",
