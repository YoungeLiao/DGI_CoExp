...
ggplot(tsne_result,aes(tSNE1,tSNE2)) + 
  geom_point(colour = '#434343', alpha = 0.5, size = 0.1) + 
...
ggplot(tsne_plot, aes(tSNE1,tSNE2,color = DGI_label)) + 
  ...
  labs(title = 'DGI (SCI = 0.194)') + # SCI = 0.1940432962014011 
  mytheme1

# save cluster assignment information

# === 4. Results visualization on DGI with subcellular ===
DGI_sub <- read.table('./03_DGI/data/24clusters/BvsD_SubCell_dgi_24cluster/types.txt', header = F) 
...
ggplot(tsne_plot, aes(tSNE1,tSNE2, color = DGI_sub_label)) + 
  ...
  theme_bw() + 
  scale_color_manual(values = pale_25) + 
  labs(title = 'Subcellular DGI (SCI = 0.193)') + 
  # SCI = 0.19256539442997084 (EM)
  mytheme
# save fig name: TSNE_BvsD_7cluster_dgi_subcel_4.8_4.5.pdf

# === 5. Kmeans results visualization ===
Kmean <- read.csv('./03_DGI/data/24clusters/Kmeans_24cluster_BvsD.csv', header = TRUE) 
...
ggplot(tsne_plot, aes(tSNE1,tSNE2,color = Kmeans_label)) + 
  ...
  theme_bw() + 
  scale_color_manual(values = pale_25) + 
  labs(title = 'Kmeans (SCI = 0.183)') + 
  # SCI = 0.18311816944012954 (Expression matrix)
  mytheme
# save fig name: TSNE_BvsD_7cluster_kmeans_4.8_4.5.pdf

# === 6. Subcellular Kmeans results visualization ===
Kmean_subcel <- read.csv('./03_DGI/data/24clusters/Kmeans_24cluster_BvsD_subcel.csv', header = TRUE) 
...
ggplot(tsne_plot, aes(tSNE1,tSNE2,color = Kmeans_subcel_label)) + 
  ...
  theme_bw() + 
  scale_color_manual(values = pale_25) + 
  labs(title = 'Subcellular Kmeans (SCI = 0.135)') + # SCI = 0.13457301691358345 (Expression matrix)
  mytheme
# save fig name: TSNE_BvsD_7cluster_kmeans_subcel_4.8_4.5.pdf

# === 6. HC ===
HC <- read.csv('./02_DatasetAnalysis/output_data/HC_24cluster_BvsD.csv', header = TRUE) 
...
ggplot(tsne_plot, aes(tSNE1,tSNE2,color = HC_label)) + 
  ...
  theme_bw() + 
  scale_color_manual(values = pale_25) + 
  labs(title = 'Hierarchical clustering') + # SCI = 0.0018333779252142142 (EM)
  mytheme1
# save fig name: TSNE_BvsD_7cluster_HC_5.6_4.5.pdf

# save data
write.csv(tsne_plot, './03_DGI/output_data/tsne_BvsD_labels_all_24cluster.csv', row.names = FALSE)
