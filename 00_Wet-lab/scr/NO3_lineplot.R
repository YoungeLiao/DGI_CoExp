# ===== 1. load data =====
library(readxl)
df1 <- as.data.frame(read_excel('./data/fig1_lineplot.xlsx', sheet = 'NO3'))
head(df1)
...
for(i in 1:length(df1$Time)){
  if(i == 1){
    nitrate <- as.numeric(df1[i,2:13])
  }
  if(i != 1){
    nitrate <- c(nitrate, as.numeric(df1[i,2:13]))
  }
}
df2$nitrate <- nitrate
head(df2)
...
attach(df2)
aov.mean<-aggregate(nitrate,by=list(Group, time),FUN=mean) # mean
aov.sd<-aggregate(nitrate,by=list(Group, time),FUN=sd) # sd
detach(df2)

plot_data <- data.frame(aov.mean, sd=aov.sd$x)
col_name <- c('Group', 'Time', 'Nitrate', 'sd')
colnames(plot_data) <- col_name
class(plot_data)
plot_data[,2:4] <- as.data.frame(apply(plot_data[,2:4], 2, as.numeric)) 

pale_4 <- 11
p <- ggplot(plot_data, aes(x=Time, y=Nitrate, group = Group)) + 
  geom_line(aes(color=Group)) +
  geom_point(aes(color=Group, shape = Group),size=4, alpha = 1) +
  geom_errorbar(aes(ymin=Nitrate-sd, ymax=Nitrate+sd, color = Group), width=3,
